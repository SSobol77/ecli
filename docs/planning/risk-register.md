...
| `vmactions/freebsd-vm` flake risk on Release pipeline | Medium | Medium | Medium | release workflow FreeBSD leg failure, SSH-wrapper exit-1 with no inner trace | pinned vmactions to commit SHA (v1.4.5), tee in-VM stdout + upload on failure, raised VM to 6 GiB / 4 vCPU; superseded by Issue #92 exact 21-asset gate, which blocks official publication until FreeBSD package and evidence assets are present | Release maintainers | Mitigated by blocking release gate |
