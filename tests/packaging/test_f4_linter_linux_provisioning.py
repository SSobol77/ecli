...
def _assert_approved_deb_mapping(
    mapping: Any,
    evidence: Any,
    package_name: str,
) -> None:
    assert mapping.mapping_status == "approved-existing-policy"
    assert mapping.provenance_status == "distro-signed-package"
    assert mapping.trust_boundary == "distro-package-manager"
    assert mapping.package_names == (package_name,)
    assert mapping.evidence_record_id == evidence.evidence_record_id
    assert mapping.evidence_source == "OS_PACKAGE_NAMES"
    assert "OS_PACKAGE_NAMES policy" in mapping.evidence_note
    assert mapping.release_blocking is False
    assert evidence.package_names == mapping.package_names


def _assert_official_deb_evidence(
    mapping: Any,
    evidence: Any,
    expected: dict[str, str],
) -> None:
    tool_id = expected["tool_id"]
    assert evidence.evidence_record_id == f"official-distro-metadata:deb:{tool_id}"
    assert evidence.evidence_source == expected["evidence_source"]
    assert evidence.evidence_source_type == "official-distro-metadata"
    assert evidence.evidence_status == "verified-official-source"
    assert evidence.official_source_name == expected["official_source_name"]
    assert evidence.official_source_url == expected["official_source_url"]
    assert evidence.official_source_kind == "distro-package-index"
    assert evidence.verification_scope == "package-name-and-executable"
    assert evidence.verified_package_names == mapping.package_names
    assert evidence.verified_executable_names == mapping.executable_names
    assert evidence.external_verification_required_for_new_mappings is False


def _assert_docker_deb_baseline_mapping(
    mapping: Any,
    evidence: Any,
    *,
    tool_id: str,
    ...
    assert mapping.distro_family == "debian"
    assert mapping.package_names == (package_name,)
    assert mapping.source_policy_artifact_entry_id == "deb"
    assert evidence.source_policy_artifact_entry_id == "deb"
    assert evidence.evidence_record_id == mapping.evidence_record_id
    assert evidence.evidence_record_id == f"repository-local-policy:deb:{tool_id}"
    assert evidence.evidence_status == "current-policy-baseline"
    assert evidence.evidence_source_type == "repository-local-policy"


def _assert_manifest_official_deb_mapping(
    tool: dict[str, Any],
    expected: dict[str, str],
) -> None:
    mapping = tool["distro_mapping"]
    evidence = mapping["evidence"]
    tool_id = expected["tool_id"]
    package_name = expected["package_name"]
    executable_name = expected["executable_name"]
    ...
    assert evidence["official_source_url"] == expected["official_source_url"]
    assert evidence["verification_scope"] == "package-name-and-executable"
    assert evidence["verified_package_names"] == [package_name]
    assert evidence["verified_executable_names"] == [executable_name]
...
def test_distro_evidence_scope_is_approved_package_mappings_only(
    ...
    promoted = [
        ...
        if record.evidence_status == "verified-official-source"
    ]
    ...
    assert all(
        ...
        for record in matrix
        if record not in promoted
    )
    assert all(
        record.evidence_source_type == "repository-local-policy"
        for record in matrix
        if record not in promoted
    )
    for artifact_id in (
...
def test_existing_os_package_policy_has_approved_distro_mapping_evidence(
    ...
    clang_tidy = _mapping_record(deb, "clang-tidy")
    clang_tidy_evidence = _evidence_record(deb_evidence, "clang-tidy")
    cppcheck = _mapping_record(deb, "cppcheck")
    ...
    _assert_approved_deb_mapping(shellcheck, shellcheck_evidence, "shellcheck")
    _assert_official_deb_evidence(
        shellcheck,
        shellcheck_evidence,
        {
    ...
    _assert_approved_deb_mapping(clang_tidy, clang_tidy_evidence, "clang-tidy")
    _assert_official_deb_evidence(
        clang_tidy,
        clang_tidy_evidence,
        {
    ...
    _assert_approved_deb_mapping(cppcheck, cppcheck_evidence, "cppcheck")
    _assert_official_deb_evidence(
        cppcheck,
        cppcheck_evidence,
        {
    ...
    _assert_approved_deb_mapping(clang_format, clang_format_evidence, "clang-format")
    _assert_official_deb_evidence(
        clang_format,
...
def test_docker_helper_distro_mappings_inherit_deb_and_rpm_policy(
    ...
    for tool_id, package_name in (
        ...
        _assert_docker_deb_baseline_mapping(
            ...
            _evidence_record(docker_deb_evidence, tool_id),
            tool_id=tool_id,
    ...
    assert rpm_yamllint_evidence.evidence_status == "current-policy-baseline"
    assert rpm_yamllint_evidence.evidence_source_type == "repository-local-policy"
...
def test_manifest_verifier_rejects_tampered_deb_yamllint_official_source_url(
    linux_helper: ModuleType,
    tmp_path: Path,
) -> None:
    manifest = _build_manifest(linux_helper, tmp_path, "deb")
    _manifest_distro_evidence(manifest)["official_source_url"] = (
    ...
    assert any(
        "yamllint: distro_mapping evidence official_source_url differs" in error
        for error in errors
    )
...
@pytest.mark.parametrize(
    ...
    (
        (
            "official_source_url",
            "https://packages.debian.org/sid/clang-tidy",
            "clang-tidy: distro_mapping evidence official_source_url differs",
        ),
        (
            "official_source_kind",
            "upstream-release-page",
            "clang-tidy: distro_mapping evidence official_source_kind differs",
        ),
        (
            "verification_scope",
            "package-name-only",
            "clang-tidy: distro_mapping evidence verification_scope differs",
        ),
        (
            "verified_package_names",
            ["wrong-package"],
            "clang-tidy: distro_mapping evidence verified_package_names differs",
        ),
        (
            "verified_executable_names",
            ["wrong-executable"],
            "clang-tidy: distro_mapping evidence verified_executable_names differs",
        ),
        (
            "evidence_source_type",
            "repository-local-policy",
            "clang-tidy: distro_mapping evidence: verified-official-source evidence cannot use repository-local-policy",
        ),
    ),
)
def test_manifest_verifier_rejects_tampered_deb_clang_tidy_official_evidence(
    linux_helper: ModuleType,
    tmp_path: Path,
    field: str,
    tampered_value: Any,
    ...
    manifest = _build_manifest(linux_helper, tmp_path, "deb")
    _manifest_distro_evidence(manifest, "clang-tidy")[field] = tampered_value
...
def test_manifest_verifier_rejects_deb_clang_tidy_without_verification_note(
    linux_helper: ModuleType,
    tmp_path: Path,
) -> None:
    manifest = _build_manifest(linux_helper, tmp_path, "deb")
    del _manifest_distro_evidence(manifest, "clang-tidy")["verification_note"]

    errors = linux_helper.verify_linux_provisioning_manifest(manifest)

    assert any(
        "clang-tidy: distro_mapping evidence: missing verification_note" in error
        for error in errors
    )
...
@pytest.mark.parametrize(
    ...
    (
        (
            "official_source_url",
            "https://packages.debian.org/sid/cppcheck",
            "cppcheck: distro_mapping evidence official_source_url differs",
        ),
        (
            "official_source_kind",
            "upstream-release-page",
            "cppcheck: distro_mapping evidence official_source_kind differs",
        ),
        (
            "verification_scope",
            "package-name-only",
            "cppcheck: distro_mapping evidence verification_scope differs",
        ),
        (
            "verified_package_names",
            ["wrong-package"],
            "cppcheck: distro_mapping evidence verified_package_names differs",
        ),
        (
            "verified_executable_names",
            ["wrong-executable"],
            "cppcheck: distro_mapping evidence verified_executable_names differs",
        ),
        (
            "evidence_source_type",
            "repository-local-policy",
            "cppcheck: distro_mapping evidence: verified-official-source evidence cannot use repository-local-policy",
        ),
    ),
)
def test_manifest_verifier_rejects_tampered_deb_cppcheck_official_evidence(
    linux_helper: ModuleType,
    tmp_path: Path,
    field: str,
    tampered_value: Any,
    ...
    manifest = _build_manifest(linux_helper, tmp_path, "deb")
    _manifest_distro_evidence(manifest, "cppcheck")[field] = tampered_value
...
def test_manifest_verifier_rejects_deb_cppcheck_without_verification_note(
    linux_helper: ModuleType,
    tmp_path: Path,
) -> None:
    manifest = _build_manifest(linux_helper, tmp_path, "deb")
    del _manifest_distro_evidence(manifest, "cppcheck")["verification_note"]

    errors = linux_helper.verify_linux_provisioning_manifest(manifest)

    assert any(
        "cppcheck: distro_mapping evidence: missing verification_note" in error
        for error in errors
    )
...
@pytest.mark.parametrize(
    ...
    (
        (
            "official_source_url",
            "https://packages.debian.org/sid/clang-format",
            "clang-format: distro_mapping evidence official_source_url differs",
        ),
        (
            "official_source_kind",
            "upstream-release-page",
            "clang-format: distro_mapping evidence official_source_kind differs",
        ),
        (
            "verification_scope",
            "package-name-only",
            "clang-format: distro_mapping evidence verification_scope differs",
        ),
        (
            "verified_package_names",
            ["wrong-package"],
            "clang-format: distro_mapping evidence verified_package_names differs",
        ),
        (
            "verified_executable_names",
            ["wrong-executable"],
            "clang-format: distro_mapping evidence verified_executable_names differs",
        ),
        (
            "evidence_source_type",
            "repository-local-policy",
            "clang-format: distro_mapping evidence: verified-official-source evidence cannot use repository-local-policy",
        ),
    ),
)
def test_manifest_verifier_rejects_tampered_deb_clang_format_official_evidence(
    linux_helper: ModuleType,
    tmp_path: Path,
    field: str,
    tampered_value: Any,
    ...
    manifest = _build_manifest(linux_helper, tmp_path, "deb")
    _manifest_distro_evidence(manifest, "clang-format")[field] = tampered_value
...
def test_manifest_verifier_rejects_deb_clang_format_without_verification_note(
    linux_helper: ModuleType,
    tmp_path: Path,
) -> None:
    manifest = _build_manifest(linux_helper, tmp_path, "deb")
    del _manifest_distro_evidence(manifest, "clang-format")["verification_note"]

    errors = linux_helper.verify_linux_provisioning_manifest(manifest)

    assert any(
        "clang-format: distro_mapping evidence: missing verification_note" in error
        for error in errors
    )
...
def test_manifest_verifier_rejects_tampered_distro_evidence_record_id(
    ...
    evidence["evidence_record_id"] = "repository-local-policy:deb:yamllint"
...
def test_manifest_verifier_rejects_tampered_deb_clang_tidy_evidence_record_id(
    linux_helper: ModuleType,
    tmp_path: Path,
) -> None:
    manifest = _build_manifest(linux_helper, tmp_path, "deb")
    evidence = _manifest_tool(manifest, "clang-tidy")["distro_mapping"]["evidence"]
    evidence["evidence_record_id"] = "repository-local-policy:deb:clang-tidy"

    errors = linux_helper.verify_linux_provisioning_manifest(manifest)

    assert any(
        "clang-tidy: distro_mapping evidence evidence_record_id differs" in error
        for error in errors
    )


def test_manifest_verifier_rejects_tampered_deb_cppcheck_evidence_record_id(
    linux_helper: ModuleType,
    tmp_path: Path,
) -> None:
    manifest = _build_manifest(linux_helper, tmp_path, "deb")
    evidence = _manifest_tool(manifest, "cppcheck")["distro_mapping"]["evidence"]
    evidence["evidence_record_id"] = "repository-local-policy:deb:cppcheck"

    errors = linux_helper.verify_linux_provisioning_manifest(manifest)

    assert any(
        "cppcheck: distro_mapping evidence evidence_record_id differs" in error
        for error in errors
    )


def test_manifest_verifier_rejects_tampered_deb_clang_format_evidence_record_id(
    linux_helper: ModuleType,
    tmp_path: Path,
) -> None:
    manifest = _build_manifest(linux_helper, tmp_path, "deb")
    evidence = _manifest_tool(manifest, "clang-format")["distro_mapping"]["evidence"]
    evidence["evidence_record_id"] = "repository-local-policy:deb:clang-format"

    errors = linux_helper.verify_linux_provisioning_manifest(manifest)

    assert any(
        "clang-format: distro_mapping evidence evidence_record_id differs" in error
        for error in errors
    )
...
def test_release_blocking_provenance_summary_tracks_current_linux_gaps(
    ...
    assert docker_evidence_summary["evidence_status_counts"] == {
        "current-policy-baseline": docker_evidence_summary["evidence_record_count"]
    }
    assert docker_promotion_summary["promotion_state_counts"] == {
        "baseline-not-promoted": docker_promotion_summary["evidence_record_count"]
    }
    assert docker_promotion_summary["verified_official_source_count"] == 0
    assert docker_promotion_summary["promotable_count"] == 0
