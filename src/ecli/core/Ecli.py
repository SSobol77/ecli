...
class Ecli:
    ...
    def exit_editor(self) -> None:  # noqa: python:S3516
        ...
        if self.is_lightweight:
            ...
            self._set_status_message("Exit action is disabled in this panel.")
            return

        # Check if the exit process is already running.
        # If yes, do nothing to avoid a repeat call.
        if hasattr(self, "_exit_in_progress") and self._exit_in_progress:
            return

        # Set the flag that we have started the exit process.
        ...
        if self.modified:
            ...
            if ans and ans.lower().startswith("y"):
                if not self.save_file():
                    # If saving failed, cancel exit.
                    self._set_status_message(
                        "Save failed or was cancelled. Exit aborted."
                    )
                    logging.warning(
                        "Exit aborted because user chose to save, but save_file() failed."
                    )

                    # Reset the flag so the user can try to exit again.
                    self._exit_in_progress = False
                    return

            elif not (ans and ans.lower().startswith("n")):
                # If the user did not press 'n' (pressed Esc or something else), cancel exit.
        ...
        # If we reached this point, the user has agreed to exit.
        ...
        try:
            # This sequence of commands returns the terminal to its normal state
            self.stdscr.keypad(False)
            curses.echo()
            curses.nocbreak()
            curses.endwin()

            if self.original_termios_attrs and sys.platform != "win32":
                try:
                    fd = sys.stdin.fileno()
                    termios.tcsetattr(
                        fd, termios.TCSADRAIN, self.original_termios_attrs
                    )
                    logging.info("Original termios attributes restored.")
                except Exception as e_term_restore:
                    logging.error(
                        "Failed to restore termios attributes: %s", e_term_restore
                    )

        except Exception as e:
            logging.exception(f"Unexpected error in exit_editor: {e}")
            logging.critical("Failed to restore terminal state cleanly.", exc_info=True)
        finally:
            # This block will be executed in any case, even if there was an error in try.
            # This is a guaranteed exit from the program.
            print("ECLI editor closed.")  # Simple message to standard output
            sys.exit(0)
    ...
    def handle_resize(self) -> bool:  # noqa: python:S3516
        ...
        try:
            ...
            self.visible_lines = max(
                1, new_height - 3
            )  # Assuming 3 lines for status/info bars
    ...
    def handle_block_indent(self) -> bool:
        ...
        with self._state_lock:
            ...
            if self.status_message == original_status:
                ...
                )  # Или "Nothing to indent in selection"
            return self.status_message != original_status
    ...
    def _process_events_and_input(self) -> bool:
        """Processes background events and user keyboard input for one loop iteration.

        This method is the first phase of the main loop. It checks for and handles
        any messages from background services (like Git, LSP, or shell commands)
        via `_process_all_queues`. It then attempts to read a key press from the
        user via the `KeyBinder`. If input is received, it is dispatched to the
        currently focused component by `_handle_input_dispatch`.
        ...
        if key_input != curses.ERR and key_input != -1:
            # 3. Delegate input handling
            if self._handle_input_dispatch(key_input):
                redraw_needed = True
