...
class LinterBridge:
    ...
    def __init__(self, editor: "Ecli") -> None:
        ...
        self.HAS_DEVOPS_LINTERS: bool = importlib.util.find_spec("lint_devops") is not None
    ...
    def run_linter(self, code: Optional[str] = None) -> bool:
        ...
        if self.devops_linter_module and hasattr(self.devops_linter_module, "DEVOPS_LINTERS") and current_lang in self.devops_linter_module.DEVOPS_LINTERS:
        ...
        if current_lang != "python":
            ...
            self.editor._set_status_message(
                message_for_statusbar=msg, is_lint_status=True, full_lint_output=msg,
    ...
    def _run_devops_linter_thread(self, language: str, code: str) -> None:
        ...
        """
        if not self.devops_linter_module: return
        try:
            result = self.devops_linter_module.run_devops_linter(language, code)
            self.editor._set_status_message(
                f"{language}: analysis complete.", is_lint_status=True,
                full_lint_output=result, activate_lint_panel_if_issues=True,
            )
        except Exception as e:
            logging.error(f"Error running DevOps linter for {language}", exc_info=True)
            self.editor._set_status_message(
                f"Error in {language} linter: {e}", is_lint_status=True,
                full_lint_output=traceback.format_exc(), activate_lint_panel_if_issues=True
            )
    ...
    def _start_lsp_server_if_needed(self) -> None:
        ...
        params = {
            ...
            "clientInfo": {"name": "Ecli"}, "workspaceFolders": [{"uri": root_uri, "name": "workspace"}]
        }
        self._send_lsp("initialize", params, is_request=True)
        self._send_lsp("initialized", {})
        self.is_lsp_initialized = True

    def _send_lsp(self, method: str, params: Optional[dict[str, Any]] = None, *, is_request: bool = False) -> None:
    ...
    def _lsp_reader_loop(self) -> None:
        ...
        while True:
            ...
            except (json.JSONDecodeError, queue.Full, Exception):
                pass
    ...
    def process_lsp_queue(self) -> bool:
        ...
        while not self.lsp_message_q.empty():
            try:
                ...
                logging.error("LSP: Error processing message from queue: %s",
                              e, exc_info=True)
        return changed
    ...
    def _handle_diagnostics(self, params: dict[str, Any]) -> None:
        ...
        if not diagnostics:
            self.editor._set_status_message(
                message_for_statusbar="✓ No issues found (Ruff)", is_lint_status=True,
        ...
        try:
            ...
            status_bar_message = f"Ruff: {msg} (Line {line})" if line > 0 else f"Ruff: {msg}"
        except Exception:
        ...
        panel_text = "\n".join(panel_lines)
        self.editor._set_status_message(
            message_for_statusbar=status_bar_message, is_lint_status=True,
            full_lint_output=panel_text, activate_lint_panel_if_issues=True,
        )
