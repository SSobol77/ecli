...
class AiResponsePanel(BasePanel):
    ...
    def draw(self) -> None:
        ...
        try:
            ...
            self._update_scroll_position(visual_cursor_y, viewport)

            # Draw visible portion of text
            visible_lines = visual_lines[
                self.visual_scroll : self.visual_scroll + viewport
            ]
            for n, (ly, start_x, chunk) in enumerate(visible_lines):
                row_y = n + 1
                self._draw_text_chunk(ly, start_x, chunk, row_y)

                # Draw cursor if it's on this line
                if (self.visual_scroll + n) == visual_cursor_y:
                    self._draw_cursor(row_y, visual_cursor_x, chunk)

            self.win.noutrefresh()
        except curses.error as e:
            logging.error(f"Curses error in CursesPanel.draw: {e}", exc_info=True)
            pass
    ...
    def _update_scroll_position(self) -> None:
        ...
        elif self.cursor_y >= self.scroll + viewport:
            self.scroll = self.cursor_y - viewport + 1
...
class FileBrowserPanel(BasePanel):
    ...
    # ecli/ui/panels.py -> class FileBrowserPanel
    ...
    def draw(self) -> None:
        ...
        # Footer section
        sep_y = self.height - 3
        self.win.hline(sep_y, 1, curses.ACS_HLINE, self.width - 2, self.attr_border)
