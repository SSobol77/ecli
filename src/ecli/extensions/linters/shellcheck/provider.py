...
#
# Project: Ecli
# File: src/ecli/extensions/linters/shellcheck/provider.py
...
"""ShellCheck diagnostics provider."""
...
_LANGUAGES = frozenset({"shellscript", "bash", "sh", "shell"})
_EXTENSIONS = frozenset({".sh", ".bash", ".ksh", ".zsh"})
_DISPLAY_NAME = "ShellCheck"
_DEFAULT_TIMEOUT_SECONDS = 15.0


class ShellCheckDiagnosticProvider:
    """Diagnostics provider backed by ``shellcheck --format=json``."""
    ...
    def supports(self, request: DiagnosticRequest) -> bool:
        """Return True for shell script files by language or extension."""
    ...
    def run(self, request: DiagnosticRequest) -> DiagnosticResult:
        """Run ShellCheck against the current file."""
        ...
        return build_command_diagnostics_result(
            ...
            timeout_seconds=self.timeout_seconds,
            parse=lambda stdout: parse_shellcheck_output(stdout, default_file=file_path),
        )
