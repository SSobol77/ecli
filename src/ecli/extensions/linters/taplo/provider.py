...
#
# Project: Ecli
# File: src/ecli/extensions/linters/taplo/provider.py
...
"""Taplo (TOML) diagnostics provider."""
...
_LANGUAGES = frozenset({"toml"})
_EXTENSIONS = frozenset({".toml"})
_DISPLAY_NAME = "Taplo"
_DEFAULT_TIMEOUT_SECONDS = 15.0


class TaploDiagnosticProvider:
    """Diagnostics provider backed by ``taplo lint``."""
    ...
    def supports(self, request: DiagnosticRequest) -> bool:
        """Return True for TOML files by language or extension."""
    ...
    def run(self, request: DiagnosticRequest) -> DiagnosticResult:
        """Run ``taplo lint`` against the current file."""
        ...
        argv = [executable, "lint", request.file_path]
        ...
        return build_command_diagnostics_result(
            ...
            timeout_seconds=self.timeout_seconds,
            parse=lambda _stdout: parse_taplo_output(
