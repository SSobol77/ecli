...
#
# Project: Ecli
# File: src/ecli/extensions/linters/zig/provider.py
...
"""Zig diagnostics provider. Zig is a first-class ECLI systems language."""
...
_LANGUAGES = frozenset({"zig"})
_EXTENSIONS = frozenset({".zig"})
_DISPLAY_NAME = "Zig"
_DEFAULT_TIMEOUT_SECONDS = 15.0


class ZigDiagnosticProvider:
    """Diagnostics provider backed by ``zig fmt --check --ast-check``.

    Never mutates files: ``--check`` is a report-only mode.
    """
    ...
    def supports(self, request: DiagnosticRequest) -> bool:
        """Return True for Zig files by language or extension."""
    ...
    def run(self, request: DiagnosticRequest) -> DiagnosticResult:
        """Run ``zig fmt --check --ast-check`` against the current file."""
        ...
        argv = [executable, "fmt", "--check", "--ast-check", request.file_path]
        ...
        return build_command_diagnostics_result(
            ...
            timeout_seconds=self.timeout_seconds,
            parse=lambda _stdout: parse_zig_output(
