...
#
# Project: Ecli
# File: src/ecli/extensions/linters/yamllint/provider.py
...
_LANGUAGES = frozenset({"yaml"})
_EXTENSIONS = frozenset({".yml", ".yaml"})
_DISPLAY_NAME = "yamllint"
_DEFAULT_TIMEOUT_SECONDS = 15.0


class YamllintDiagnosticProvider:
    """Diagnostics provider backed by ``yamllint --format parsable``."""
    ...
    def supports(self, request: DiagnosticRequest) -> bool:
        """Return True for YAML files by language or extension."""
    ...
    def run(self, request: DiagnosticRequest) -> DiagnosticResult:
        """Run yamllint against the current file."""
        ...
        argv = [executable, "--format", "parsable", request.file_path]
        ...
        return build_command_diagnostics_result(
            ...
            timeout_seconds=self.timeout_seconds,
            parse=lambda stdout: parse_yamllint_output(stdout, default_file=file_path),
        )
